...
This folder contain the Infrastructure as Code or Terraform code to create an IBM Cloud Catalog to create a **Sandbox** with an **Openshift** (ROKS) cluster on IBM Cloud **Classic** or **VPC** Gen 2, with **Multi Cloud Management Cloud Pak** (CP4MCM), **Cloud Pak for Data** (CP4DATA) or **Applications Cloud Pak** (CP4App)

The sandbox Openshift cluster with CP4DATA or CP4App is provisioned using the Private Catalog on IBM Cloud Web Console, however this documentation include instructions to get the cluster on the CLI using Terraform, Schematics or the IBM Cloud CLI.
...
- [Development of the Private Catalog to create a Cloud Pack Sandbox](#development-of-the-private-catalog-to-create-a-cloud-pack-sandbox)
  ...
  - [Cloud Pak for Multi Cloud Management (CP4MCM)](#cloud-pak-for-multi-cloud-management-cp4mcm)
    ...
    - [CP4DATA Validation](#cp4data-validation)
  - [Cloud Pak for Applications (CP4Apps)](#cloud-pak-for-applications-cp4apps)
    ...
    - [CP4DATA Validation](#cp4data-validation-1)

## Private Catalog Deployment

To release a new version of the Private Catalog execute the `make` command to create the file `product/CPS-DATA-1.x.y.tgz` file with all the code required for the Catalog Product.
...
The quickest way to have a ROKS cluster up and running either empty, with CP4DATA or with CP4APP, on IBM Cloud Classic, Dallas region and _cloud-pak-sandbox_ resource group, is executing `make with-terraform` like that or appending the suffix `-data` or `-app` respectively, like so:
...
Set the following variables in the `terraform.tfvars` file, using the `region` and Openshift version (`roks_version`) from the previously executed commands.
...
Export the credentials for IBM Cloud Classic, identify the zone (datacenter) to install the cluster and the machine type (flavor).
...
Set the following variables in the `terraform.tfvars` file, using the `datacenter` and `flavor` from the previously executed commands. The `size` is the number of workers to create:
...
- With Cloud Pak for **Multi Cloud Management**:
  ...
  Open the `terraform.tfvars` file to set to `true` the variable `with_cp4mcm`. Optionally, modify the boolean values of the following DATA modules variables to enable or disable them:
...
- With Cloud Pak for **Data**:
  ...
  Open the `terraform.tfvars` file to set to `true` the variable `with_cp4data`.
...
If CP4DATA was enabled, open the CP4DATA Endpoint using the following URL and credentials:
...
Other set of variables to assign values are the DATA modules to enable or disable. Set `true` or `false` to the following DATA modules variables:
...
The following table contain the input variables required to install CP4DATA:
...
| `infr_mgt_install`             | Enable or disable the CP4DATA Module Infrastructure Management                                                                                                   | `false`              | No       |
| `monitoring_install`           | Enable or disable the CP4DATA Module Monitoring                                                                                                                  | `false`              | No       |
| `security_svcs_install`        | Enable or disable the CP4DATA Module Security Services                                                                                                           | `false`              | No       |
| `operations_install`           | Enable or disable the CP4DATA Module Operations                                                                                                                  | `false`              | No       |
| `tech_prev_install`            | Enable or disable the CP4DATA Module Tech Prev                                                                                                                   | `false`              | No       |
...
Execute the following commands to validate DATA:
