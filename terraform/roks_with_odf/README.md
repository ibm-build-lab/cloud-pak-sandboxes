...
| `datacenter`           | **IBM Cloud Classic** only (`on_vpc` = `false`). Ignored if `cluster_id` is specified. This is the Datacenter or Zone in the Region to provision the cluster. List all available zones with: `ibmcloud ks zone ls --provider classic`    | `tor01`          | No       |
