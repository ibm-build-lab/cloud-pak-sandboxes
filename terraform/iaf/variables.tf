...
variable "flavors" {
  type        = list(string)
  default     = ["b3c.16x64"]
  description = "Array with the flavors or machine types of each the workers group. Classic only takes the first flavor of the list. List all flavors for each zone with: 'ibmcloud ks flavors --zone us-south-1 --provider <classic | vpc-gen2>'. Example: [\"bx2.16x64\", \"mx2.8x64\", \"cx2.4x8\"]"
}

// Only required if cluster id is not specified and 'on_vpc=false'
variable "datacenter" {
  default     = ""
  description = "Classic Only: Datacenter or Zone in the region to provision the cluster. List all available zones with: 'ibmcloud ks zone ls --provider classic'. Only required if cluster id not specified and on_vpc=false."
}
...
variable "private_vlan_number" {
  default     = ""
  description = "Classic Only: Private VLAN assigned to your zone. List available VLANs in the zone: 'ibmcloud ks vlan ls --zone <datacenter>', make sure the the VLAN type is private and the router begins with bc. Use the ID or Number. Only required if cluster id not specified and on_vpc=false."
}
variable "public_vlan_number" {
  default     = ""
  description = "Classic Only: Public VLAN assigned to your zone. List available VLANs in the zone: 'ibmcloud ks vlan ls --zone <datacenter>', make sure the the VLAN type is public and the router begins with fc. Use the ID or Number. Only required if cluster id not specified and on_vpc=false."
}
...
variable "vpc_zone_names" {
  type        = list(string)
  default     = ["us-south-1"]
  description = "VPC Only: Zones in the IBM Cloud VPC region to provision the cluster. List all available zones with: 'ibmcloud ks zone ls --provider vpc-gen2'. Only required if cluster id not specified and on_vpc=true."
}
